...
async function makeRequest(request: JobRequest) {
    const claimNextRetryAt = calculateNextRetry(request.backoff_ms, request.attempt_count);
    ...
    try {
        ...
        if (res.ok) {
            ...
            const postponeNextRetryAt = calculateNextRetry(request.backoff_ms, newAttemptCount);
...
const calculateNextRetry = (backoffMs: number, attemptCount: number): number => {
